...
<article class="{$component} {cmods name=$component mods=$mods} {$classes}" {cattr list=$attributes}>
    ...
    <header class="{$component}-header">
		...
		{* Дата *}
			<time datetime="{date_format date=$topic->getDatePublish() format='c'}" title="{date_format date=$topic->getDatePublish() format='j F Y, H:i'}" class="ls-time-published">
                        ...
                        {if $isList && ( ! $topic->getForbidComment() || ( $topic->getForbidComment() && $topic->getCountComment() ) )}
                            <li class="{$component}-info-item {$component}-info-item--comments {if $_countCommentNew}{$component}-info-item--comments--has-new{/if}">
                                <a href="{$topic->getUrl()}#comments">
                                    {if $topic->getCountComment() == 0}
										{$aLang.comments.commenting}
									{else}
                        ...
                        {if ! $isPreview && !$isList}
                            ...
                            <li class="{$component}-info-item {$component}-info-item--share">
								{hookb run="topic_share" topic=$topic isList=$isList}
									<div class="yashare-auto-init" data-yashareTitle="{$topic->getTitle()|escape}" data-yashareLink="{$topic->getUrl()}" data-yashareL10n="ru" data-yashareType="big" data-yashareTheme="counter" data-yashareQuickServices="yaru,vkontakte,facebook,twitter,odnoklassniki,moimir,gplus"></div>
								{/hookb}
							</li>
						{/if}
                    {/block} {* /topic_footer_info_items *}
